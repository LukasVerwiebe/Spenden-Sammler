 ...
 * @author lu80238
 */
@Named(value="orgaEinkommenModel")
@RequestScoped
public class OrgaEinkommenModel {
    ...
    @PostConstruct
    public void init() {
        if(sessiondata.getBenutzer().getRolleBenutzer().equals("Organisation")) {
            einkommenListe = userController.findAllEinkommenOrga(sessiondata.getBenutzer().getCharity().getIdCharity());
        }
    }
    
    public void einkommenliste() {
        if(sessiondata.getBenutzer().getRolleBenutzer().equals("Organisation")) {
            einkommenListe = userController.findAllEinkommenOrga(sessiondata.getBenutzer().getCharity().getIdCharity());
        }
    }
