 ...
 * @author Lukas
 */
@Named(value="benutzerQuittungenModel")
@ViewScoped
public class BenutzerQuittungenModel implements Serializable{
    ...
    @Inject
    private QuittungAnsehenModel quittungAnsehenModel;
    ...
    @PostConstruct
    public void init() {
        if(sessiondata.getBenutzer().getRolleBenutzer().equals("Benutzer")) {
            quittungenListe = userController.findAllQuittungenBenutzer(sessiondata.getBenutzer().getIdBenutzer());
        }
    }
    
    public String showSelectedQuittung(Quittung quittung) {
        quittungAnsehenModel.setSelectedQuittung(quittung);
